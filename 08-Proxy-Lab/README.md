...
一定要注意， `tiny.c` 在 `./tiny` 目录下。

如上命令使用 `make` 命令编译 `tiny` 服务器（以括号包裹命令，不更改 $PWD 工作目录），然后使用 `.tiny/tiny 7778` 命令启动内容服务器，其中 `7778` 是内容服务器端口号，如果已经被占用，你可以自行修改。
...
一个比较奇葩的事情是，我们上面说了考虑了这么多读者啊写者啊的问题，但是经测试，直接简单地对每个操作使用一个 `mutex` 锁，直接上一把大锁，照样能够满分，这可能是我们要处理的缓存内容实在是太小了，所以实际上即使是这么粗犷的方式，也不会出现什么问题。
