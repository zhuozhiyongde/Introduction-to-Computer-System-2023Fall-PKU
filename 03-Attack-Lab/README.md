...
*(后续经过尝试，发现覆盖`getbuf`返回地址以及栈中更早的信息不会影响程序后续进行，因为成功调用 `touch3`后，程序就不会再返回到先前的函数，而原有的栈帧也没有保留的意义了)*
...
*更正:README文件中提示命令行中需要加 `-Og`，从而使 `farm.o`反汇编后与 `rtarget.s`的 `farm`部分相同*
```bash
gcc -c -Og farm.c
objdump -d farm.o > farm.s
```
